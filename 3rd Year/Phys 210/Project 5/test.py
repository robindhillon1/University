...
for i in range(steps):
    ...
    if (i*dt) <= 10:  # extracts standard deviation at specific times, which
        ...
        popt, pcov = curve_fit(gauss, xvalues[::3], C_slice[::3])
...
plt.plot(times[10::34], stds[10::34], 'bo', markersize = 2.5, label = 'Data')
plt.plot(np.sqrt(theory[10::34]), theoryLine[10::34], 'k-', label='Theory')
plt.xlabel('Square Root of Time')
plt.ylabel('Standard Deviation')
plt.title('Evolution of Standard Deviation Vs. Time')
plt.grid()
plt.legend()
...
plt.xlabel('x(m)')
plt.ylabel('y(m)')
plt.title('Evolution of Diffusion Equation, C(x,y,t) from t=[0,100]s')
