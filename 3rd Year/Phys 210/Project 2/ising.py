...
# I could import numba and use @jit to make the code run faster, but it doesn't
# work on the computers in the lab on campus. So I decided not to use that in
# case the code doesn't then work when you check. However, code is faster when
# I use numba.
...
def new_config(grInit):
    ...
    i = np.random.randint(0, 50)  # creating a random number in range [0,50]
    j = np.random.randint(0, 50)  # creating a random number in range [0,50]
    grNew[i, j] = -1*grNew[i, j]  # using the 2 random numbers above, indexing
    # into the grid, and flipping the sign at that index.
    return grNew, grInit, i, j  # returns new grid, old grid, row, column.
...
def animations(grInit, Temp):
    ...
    for i in grids:  # iterating through the list of grids
        ...
        # with chosen colormap. Also tried imshow, but I like pcolormesh better
        # even if the only difference is that imshow displays sqaure animation.
...
for i in Temps:
    mov = animations(grInit, i)  # calls above function and receives animation
    mov.save('temp_{}.mp4'.format(i))  # saves it to corresponding temperature
    plt.show()  # I could have simply repeated this block of code each time for
    # the corresponding temperatures, but this is cleaner and more convenient.
